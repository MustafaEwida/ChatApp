...
class Rsest_Email extends StatefulWidget {
 

  @override
  _Rsest_EmailState createState() => _Rsest_EmailState();
}

class _Rsest_EmailState extends State<Rsest_Email>  with SingleTickerProviderStateMixin{
...
actions:<Widget>[
    ...
    if (!text.contains('@') || !text.contains('.com')/*||!val.contains('hotmail')||!val.contains('gmail')*/) {
                          return  showDialog(context: context, builder: (ctx){
return AlertDialog(
title: Text("Reset Faild"),
...
content: Text('Rsest done successfully,please press ok to back to log screen to log with the new email'),
actions:<Widget>[
              FlatButton(
                child: Text('Ok'),
                onPressed: () {
...
style:  ElevatedButton.styleFrom(
  ...
  shape: RoundedRectangleBorder(
          ...
          onPressed: (){
            onsubmit(tec.text);
          }, child: Text("Check",style: TextStyle(fontSize: 20.sp),)),), SizedBox(height: 20,),
      ],
     ),),
       if(!isuser) SizedBox(height: 152.h,),
if(isuser) 
