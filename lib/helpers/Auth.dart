...
Timer? timer ;
  ...
  sginup(UserMdoel UserMdoel) async {
    try {
      ...
      /*path ='https://wellcome-ec07c-default-rtdb.firebaseio.com/users/${user!.id}.json';
...
"name":UserMdoel.name,
 ...
 if(timer!=null){
  ...
  Future<bool> TryLogIn() async {
    ...
    final data = json.decode(SP.getString("data")!) as Map<String, dynamic>;
...
Reset(String email)async{
   final dio = Dio();
final path = 'https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyC9DDF6YMhlf-UwemVpsCU_tg-0NNM9W_A';
...
changeEmail(String email) async{
  final dio = Dio();
var path = 'https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyC9DDF6YMhlf-UwemVpsCU_tg-0NNM9W_A';
...
expireDate = DateTime.now().add(Duration(seconds: secs ));
 path ='https://wellcome-ec07c-default-rtdb.firebaseio.com/users/${user!.id}.json?auth=$Token';
...
deleteAccount()async{
    final dio = Dio();
try {
  var path = "https://identitytoolkit.googleapis.com/v1/accounts:delete?key=AIzaSyC9DDF6YMhlf-UwemVpsCU_tg-0NNM9W_A";
await dio.post(path,data: {
  "idToken":Token
});
 path ='https://wellcome-ec07c-default-rtdb.firebaseio.com/users/${user!.id}.json?auth=$Token';
 ...
 logout();
   ...
   print(e.response!.data);
  throw MyEx(e.response!.data['data']['message']);
